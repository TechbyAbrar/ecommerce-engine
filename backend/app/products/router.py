...
from app.products.dependencies import require_admin
...
async def create_product(data: ProductCreate, _: object = Depends(require_admin), db: AsyncSession = Depends(get_db)):
    return SuccessResponse(message="Product created", data=await ProductService(db).create(data))


@router.patch("/{product_id}", response_model=SuccessResponse[ProductRead], tags=["Admin Products"])
async def update_product(product_id: uuid.UUID, data: ProductUpdate, _: object = Depends(require_admin), db: AsyncSession = Depends(get_db)):
    return SuccessResponse(message="Product updated", data=await ProductService(db).update(product_id, data))
